...
$(function () {
  ...
  function onFocusChanged(systemName) {
    console.log('onFocusChange ... ', systemName);

    const nameElement = document.getElementsByClassName(
      'overlay-content__name',
    )[0];
    ...
    nameElement.innerHTML = systemName;
    descriptionElement.innerHTML =
      'put your description for this object here ...' + systemName;

    $('#lookat-dropdown-label').text('Look At: ' + systemName);
    $('#lookAtDropdownOptions > li > a').css('font-weight', 'normal');
    $("#lookAtDropdownOptions > li > a:contains('" + systemName + "')").css(
      'font-weight',
      'bold',
    );
  }
  ...
  $('#view-from-above').click(function (e) {
    ...
    $("#lookFromDropdownOptions > li > a:contains('Above')").css(
      'font-weight',
      'bold',
    );
    e.preventDefault();
  ...
  $('#view-from-ecliptic').click(function (e) {
    ...
    $('#lookFromDropdownOptions > li > a').css('font-weight', 'normal');
    $("#lookFromDropdownOptions > li > a:contains('Ecliptic')").css(
      'font-weight',
      'bold',
    );
    e.preventDefault();
  ...
  $('#view-from-earth').click(function (e) {
    ...
    $('#lookFromDropdownOptions > li > a').css('font-weight', 'normal');
    $("#lookFromDropdownOptions > li > a:contains('Earth')").css(
      'font-weight',
      'bold',
    );
    e.preventDefault();
  ...
  $('#view-from-default').click(function (e) {
    ...
    $('#lookFromDropdownOptions > li > a').css('font-weight', 'normal');
    $("#lookFromDropdownOptions > li > a:contains('Default')").css(
      'font-weight',
      'bold',
    );
    e.preventDefault();
  ...
  function updateUiDateInputs() {
    if (orbitViewer) {
      updateUiDateInputsWith(
        ...
        moment(orbitViewer.getSimulationTimeMillis()).format(
          JPL.Config.datePicker.format,
        ),
      );
    } else {
      updateUiDateInputsWith(
        JPL.Util.julianToDate(JPL.Util.julianNow(true)),
        moment(JPL.Util.julianToDate(JPL.Util.julianNow(true))).format(
          JPL.Config.datePicker.format,
        ),
      );
    }
  ...
  $('#floating-datetime-show').popover({
    ...
    container: 'body',
    content:
      '<input type="text" id="datetimepicker-input" class="form-control" placeholder="YYYY-MM-DD HH:mm" ><div id="datetimepicker-inline" style=\'width:400px;\'></div>',
  });
  ...
  $('#floating-datetime-show').click(function () {
    ...
    $('#datetimepicker-inline').on('dp.change', function (e) {
      ...
      $('#datetimepicker-input').val(
        moment(e.date).format(JPL.Config.datePicker.format),
      );
    });
  ...
  orbitViewer = new JPL.OrbitViewer({
    ...
    onPrimaryBodyLoaded: function (system) {
      ...
      if (
        (system.getName() !== 'Sun' &&
          system.getSystemMainBodyType() == 'planet') ||
        system.getName() == 'Pluto'
      ) {
  ...
  if (elem) {
    orbitViewer.loadElementsString(
      elem,
      onSmallBodyLoaded,
      function (elem, err) {
        console.info(
          "Element string property '" + err + "' missing or invalid",
        );
      },
    );
  }
  ...
  if (showDistanceLabels !== undefined) {
    ...
    $('#distance-labels-toggle').bootstrapToggle(
      showDistanceLabels ? 'on' : 'off',
    );
  }
  ...
  if (showFloatingControls !== undefined) {
    $('#floating_controls').css(
      'display',
      showFloatingControls ? 'block' : 'none',
    );
    $('#show-controls-toggle').bootstrapToggle(
      showFloatingControls ? 'on' : 'off',
    );
  }
  ...
  function onAddObjectSubmit() {
    ...
    if (search.match(/[0-9]+ [A-Za-z]+ \([0-9]{4} [A-Z0-9]{2,5}\)/)) {
      // Full SBDB Name
      search = /^[0-9]+/.exec(search)[0];
    }
    orbitViewer.loadSmallBody(
      search,
  ...
  function setSystemOptionsEnabled(system, enabled) {
    if (system) {
      ...
      $('#' + system.getName().hashCode() + '-label').attr(
        'disabled',
        enabled ? null : 'true',
      );
  ...
  function createSystemOptions(system) {
    ...
    optionsContainer.append(
      $('<div/>')
        .addClass('system-options-primary')
        .append(
          createSystemOptionsCheck(
            system.getName(),
            true,
            system.getName().hashCode() + '-primary',
            function (evt) {
              if (this.checked) {
                $('#' + system.getName().hashCode() + '-moons').attr(
                  'disabled',
                  null,
                );
                $('#' + system.getName().hashCode() + '-orbit').attr(
                  'disabled',
                  null,
                );
                $('#' + system.getName().hashCode() + '-label').attr(
                ...
                $('#' + system.getName().hashCode() + '-droplines').attr(
                  'disabled',
                  null,
                );

                system.setMoonsVisible(
                  $('#' + system.getName().hashCode() + '-moons').prop(
                    'checked',
                  ),
                );
                system.setOrbitVisible(
                  $('#' + system.getName().hashCode() + '-orbit').prop(
                    'checked',
                  ),
                );
                system.setBodyLabelVisible(
                  $('#' + system.getName().hashCode() + '-label').prop(
                    'checked',
                  ),
                );
                system.setOrbitDropLinesVisible(
                  $('#' + system.getName().hashCode() + '-droplines').prop(
                    'checked',
                  ),
                );
              } else {
    ...
    if (system.getMoons().length > 0) {
      var check = createSystemOptionsCheck(
        system.getMoons().length == 1 ? 'Moon' : 'Moons',
        system.areMoonsVisible(),
        system.getName().hashCode() + '-moons',
        function (evt) {
    ...
    if (system.isCustomLoadedObject()) {
      subOptions.append(
        createSystemOptionsCheck(
          'Drop Lines',
          true,
          system.getName().hashCode() + '-droplines',
          function (evt) {
